...
public class IdentityHelper {
	 ...
	 * 获得下一个流水号
	 * @param current 当前流水号
	 * @param beginIndex 流水号变化部分的起始位置，默认从0开始
	 * @param endIndex	流水号变化部分的截止位置(不包括)，默认最后
	 * @param hex	是否十六进制：true是，false否
	 * @return 下一个流水号
	 */
	public static String nextSerialNo(String current, Integer beginIndex, Integer endIndex, Boolean hex) {
		if (current==null) return null;
		int length = current.length();
		if (beginIndex==null || beginIndex.intValue() < 1 || beginIndex.intValue() >= length) beginIndex = 0;
		if (endIndex==null || endIndex.intValue() > length) endIndex = length;
		if (endIndex.intValue() <= beginIndex.intValue()) endIndex = beginIndex.intValue() + 1;

		String before = current.substring(beginIndex, endIndex);
		int ii = (hex!=null && hex.booleanValue()==true ? Integer.parseInt(before,16) : Integer.parseInt(before)) + 1;
		String after = hex!=null && hex.booleanValue()==true ? Integer.toHexString(ii).toUpperCase() : String.valueOf(ii);

		StringBuffer sbf = new StringBuffer(50);
		if ((endIndex.intValue() - beginIndex.intValue()) < after.length()) {
			sbf.append("SerialNo ").append(after).append(" out of length : ").append(endIndex.intValue() - beginIndex.intValue());
