...
public class VerifyCodeController extends BaseController{
	...
	public void getVerifyImage(HttpServletResponse response,String common) throws Exception {  
		...
		if("login".equals(common)){
			 cookie = new Cookie(Constant.HXP_MANAGER_LOGIN_VERFYCODE_KEY,EncryptionUtil.MD5(sRand));
		}else if ("back".equals(common)){
			 cookie = new Cookie(Constant.HXP_MANAGER_BACKPWD_VERFYCODE_KEY,EncryptionUtil.MD5(sRand));
		}else if("register".equals(common)){
			 cookie = new Cookie(Constant.HXP_MANAGER_REG_VERFYCODE_KEY,EncryptionUtil.MD5(sRand));
		}
	    cookie.setPath("/");
